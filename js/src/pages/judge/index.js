            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-around' }}>
                {this.state.myCompetition.map(com => {
                    return <div style={{ margin: 20, display: 'inline-block' }}>
                        <div style={{
                            padding: 20, background: '#fff', borderRadius: 6,
                            boxShadow: '1px 1px 3px #aaa',
                            maxWidth: 430, width: '90vw', color: '#555', lineHeight: 2
                        }} key={com.competition_id + com.judge_user_id}>
                            ...
                            <div>作品{com.count}个，{com.review}个已打分  {com.count === 0 ? '' : <span style={{ float: 'right' }}>进度{com.review / com.count * 100}%</span>}</div>
                            <div style={{ textAlign: 'right', marginTop: 20 }}>{
                                com.competition_state === 0 ? com.count === 0 ? '未开始' : <Link to={'/judge/' + com.judge_id}><Button type="primary" >去打分</Button></Link> :
