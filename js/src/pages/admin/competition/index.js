...
class Home extends React.Component {
    ...
    limit = 10;
    columns = [
        { key: 'competition_id', title: '比赛名', render: d => <Link to={`competition/${d.competition_id}`}>{d.competition_name}</Link> },
        { title: '描述', render: d => <Tooltip title={d.competition_desc}>{d.competition_desc.substr(0, 10) + '...'}</Tooltip> },
        { title: '时间描述', dataIndex:'competition_time'},
    ...
    render() {
        return <div>
            ...
            <Modal
                title="创建比赛"
                okButtonProps={{ loading: this.state.okLoading }}
                onOk={this.addOk}
                destroyOnClose
